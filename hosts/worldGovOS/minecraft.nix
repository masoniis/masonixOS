  ...
  systemd.services.astral = {
    ...
    serviceConfig = {
      ExecStartPre = let
        ...
        '';
      in "${pkgs.bash}/bin/bash -c 'cp -f ${configTemplate} /data/minecraft/astral/lazymc.toml && chmod +w /data/minecraft/astral/lazymc.toml'";
      ...
      ExecStart = let
        ...
        lazymcOld = pkgs.stdenv.mkDerivation {
          ...
          src = pkgs.fetchurl {
            url =
              "https://github.com/timvisee/lazymc/releases/download/v0.2.7/lazymc-v0.2.7-linux-x64-static";
      ...
      in "${lazymcOld}/bin/lazymc --config /data/minecraft/astral/lazymc.toml start";
    ...
    serviceConfig = {
      ExecStartPre = let
        configTemplate = pkgs.writeText "lazymc.toml" ''
          ...
          rewrite_server_properties = true # lazymc updates properties with rcon password, ports, etc
        '';
      in "${pkgs.bash}/bin/bash -c 'cp -f ${configTemplate} /data/minecraft/astral-creative/lazymc.toml && chmod +w /data/minecraft/astral-creative/lazymc.toml'";
      ...
      in "${lazymcOld}/bin/lazymc --config /data/minecraft/astral-creative/lazymc.toml start";
      ...
      # Startup/restart handling
      TimeoutStopSec =
        "1500"; # give ~15 minutes before force killing on a shutdown
      Restart = "on-failure";
      RestartSec = "20s";
